...
class RecommendationEngine(private val context: Context) {
    ...
    private val GEMINI_API_KEY = getGeminiApiKey()
    ...
    private fun getGeminiApiKey(): String {
        // Try to read from .env file, fallback to hardcoded key
        return try {
            val envFile = context.assets.open(".env")
            val content = envFile.bufferedReader().use { it.readText() }
            val keyLine = content.lines().find { it.startsWith("GEMINI_API_KEY=") }
            val apiKey = keyLine?.substringAfter("=")?.trim() ?: "AIzaSyDQTNqMEHEMicuAgZluk2SZuXRPBEplJWE"
            Log.d(TAG, "Using Gemini API key from .env file")
            apiKey
        } catch (e: Exception) {
            Log.w(TAG, "Could not read .env file, using default API key", e)
            "AIzaSyDQTNqMEHEMicuAgZluk2SZuXRPBEplJWE"
        }
    }
